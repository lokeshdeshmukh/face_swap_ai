...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Portrait reenactment backend using LivePortrait")
...
def main() -> None:
    ...
    if shutil.which("liveportrait") is None:
        raise SystemExit("liveportrait executable not found in worker image")
    ...
    if shot_plan.task_type != "portrait_reenactment":
        raise SystemExit(f"portrait_reenactment backend expected task_type=portrait_reenactment, got {shot_plan.task_type}")
    ...
    with tempfile.TemporaryDirectory(prefix="liveportrait-reenactment-") as temp_dir:
        temp_path = Path(temp_dir)
        animated = temp_path / "animated.mp4"
        cmd = [
            "liveportrait",
            "--source-image",
            str(source_image),
        ...
        if not animated.exists() or animated.stat().st_size <= 0:
            raise SystemExit("liveportrait produced no output video")

        shutil.copyfile(animated, output_path)
